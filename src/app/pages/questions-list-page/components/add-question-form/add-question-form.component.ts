...
export class AddQuestionFormComponent {
  ...
  constructor(
    ...
    this.questionService.getAllCategories().subscribe((res) => {
      ...
      if (this.data.formType.includes('UPRAVIŤ')) {
        ...
        this.questionForm.controls['q_id'].setValue(this.data.question.id);
      }
    });
  }
  ...
  onSubmit(): void {
    const question: any = {
      ...
      category:
        this.questionForm.controls['category'].value !== 'Iná'
          ? this.questionForm.controls['category'].value
          : this.questionForm.controls['new_category'].value,
    };

    if (this.data.formType.includes('UPRAVIŤ')) {
      question.id = this.data.question.id;
      this.questionService.updateQuestionById(question).subscribe({
        next: (success) => {
          this.alertService.success(
            'Otázka bola úspešne aktualizovaná.',
            'Výborne!',
          );
        },
        error: (err) => {
          this.alertService.error(
            'Nepodarilo sa aktualizovať otázku.',
            'Nastala chyba!',
          );
        },
      });
      ...
      this.questionService.addNewQuestion(question).subscribe({
        next: (success) => {
          this.alertService.success(
            'Otázka bola úspešne vytvorená.',
            'Výborne!',
          );
        },
        error: (err) => {
          this.alertService.error(
            'Nepodarilo sa vytvoriť otázku.',
            'Nastala chyba!',
          );
        },
      });
